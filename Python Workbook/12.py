...
t1 = radians(float(input("Insert t1 (latitude of first point): ")))
g1 = radians(float(input("Insert g1 (longitude of first point): ")))
t2 = radians(float(input("Insert t2 (latitude of second point): ")))
g2 = radians(float(input("Insert g2 (longitude of second point): ")))

distance = 6371.01 * acos(sin(t1) * sin(t2) + cos(t1) * cos(t2) * cos(g1 - g2))             # distance = 6371.01 × arccos(sin(t1) × sin(t2) + cos(t1) × cos(t2) × cos(g1 − g2))
print(f"Distance is {distance:.4f} km")
